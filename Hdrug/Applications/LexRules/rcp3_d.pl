...
count(B,D,F) :-
	hdrug_util:count_edges(rcp3_d:inactive(_,_,_),B),
	hdrug_util:count_edges(rcp3_d:tt(_,_,_,_,_,_),D),
	hdrug_util:count_edges(rcp3_d:tt(_,_,_),F).
...
apply_semantics(Mother,P0,P,tree(Name,_,[])) :-
	tt(P0,P,Name),
	user:lex(P0,P,Mother,Name).
apply_semantics(Mother,P0,P,tree(Name,_,Trees)):-
