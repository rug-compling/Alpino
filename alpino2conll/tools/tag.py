...
def retag(intabfn, outtabfn=None):
		...
		if fields[0] == '\n':
			...
			if len(tags) == len(lines):
				...
				print >>sys.stdout, "done. exit. ", outtabfn

			else:
				print >>sys.stdout, "LENGTH MISMATCH: split/join"
                                #split or join
				t = 0
				for fields in lines:
...
def isValidEnding(word,longword):
		...
		listrest = filter(lambda m: m != "",tmp_listrest)
		print listrest
		if len(listrest) == 0:
			return False
		#if len(rest) == 0:
	...
	sys.stderr.write('Error: no ALPINO_HOME set!\n')
        ...
        for intabfn in args:
                if options.file:
                        ...
                        print >>sys.stderr, 'retagging %s to %s' % (intabfn, outtabfn)	    
                        retag(intabfn, outtabfn)
                else:
                        print >>sys.stderr, 'retagging', intabfn	    
                        retag(intabfn)
