...
if sys.stdin.isatty() and len(sys.argv) < 2:
    ...
    find /net/corpora/Lassy/SoNaRCorpus_NC_1.2/SONAR500/FoLiA/ -name '*.folia.xml' -print | {} 
...
#                lemma = word.find('./{http://ilk.uvt.nl/folia}lemma').attrib['class']
                sys.stdout.write('{}{}'.format(sep, escape(text)))
                sep = ' '
            sys.stdout.write('\n')
    except:
