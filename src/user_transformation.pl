...
surf_lemma(laatste,'TW(rang,prenom,stan)','ADJ(prenom,sup,met-e,stan)',laatste,laat).
surf_lemma(laatste,'TW(rang,nom,zonder-n)','ADJ(nom,sup,met-e,zonder-n,stan)',laatste,laat).
...
mwu_words(Words,_,Words,_,['SPEC(deeleigen)','SPEC(deeleigen)']) :-
    mwu_pair(Words).

mwu_words(Words,_,Words,_,['SPEC(deeleigen)','SPEC(deeleigen)','SPEC(deeleigen)']) :-
    mwu_triple(Words).

mwu_triple(_):-fail.
%mwu_triple(['Raad',van,'State']).

mwu_pair(_):-fail.
%mwu_pair(['Hoge','Raad']).
%mwu_pair(['Europese','Commissie']).
