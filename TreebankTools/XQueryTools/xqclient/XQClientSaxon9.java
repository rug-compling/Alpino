/*
  XQClientSaxon9.java: a minimal XQuery processor that uses
  Saxon internals and retrieves documents from a treebank server.

  XQClientSaxon9.java [-h host] [-p port] <queryfile|{query}> <--stdin|filename(s)>

  Geert Kloosterman <g.j.kloosterman@gmail.com>
  Sat Dec  2 10:30:01 2006

  Sun Apr  4 19:33:14 2010: updated for Saxon 9
*/
...
public class XQClientSaxon9 extends Query
{
    static String name = "xqclient";

    public static void main(String args[])
            throws Exception {
        // the real work is delegated to another routine to allow subclassing
        (new XQClientSaxon9()).doMain(args);
    }
    ...
    public void doMain(String args[])
    {
        ...
        config = makeConfiguration(false, null);
        config.setHostLanguage(Configuration.XQUERY);
        StaticQueryContext staticContext = new StaticQueryContext(config);
        DynamicQueryContext dynamicContext = new DynamicQueryContext(config);
        XQueryExpression exp = null;

        // compile the query (we could probably use the inherited compileQuery())
        try {
        ...
        if (useStdin) {
            ...
            try {
                while ((line = reader.readLine()) != null) {
                    fileName = line;
                    processFile(client, fileName, exp, props, dynamicContext);
                }
            } catch (IOException e) {
                e.printStackTrace();
                System.exit(1);
    ...
    protected void processFile(TreebankClient client, 
                               String fileName, 
