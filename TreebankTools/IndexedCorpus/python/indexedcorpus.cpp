...
typedef struct {
  PyObject_HEAD indexedcorpus::IndexedCorpusReader *indexedCorpusReader;
} IndexedCorpusReader;
...
static PyObject *IndexedCorpusReader_data(IndexedCorpusReader *self,
                                          PyObject *args) {
  char *name;
...
static PyObject *IndexedCorpusReader_entries(IndexedCorpusReader *self) {
  ...
  std::vector<std::string> const &entryVec =
      self->indexedCorpusReader->entries();
...
static PyMethodDef IndexedCorpusReader_methods[] = {
    {"data", (PyCFunction)IndexedCorpusReader_data, METH_VARARGS,
     "Get corpus data"},
    {"entries", (PyCFunction)IndexedCorpusReader_entries, METH_NOARGS,
     "List corpus entries"},
    {NULL} /* Sentinel */
};
...
typedef struct {
  PyObject_HEAD indexedcorpus::IndexedCorpusWriter *indexedCorpusWriter;
} IndexedCorpusWriter;
...
static PyObject *IndexedCorpusWriter_write(IndexedCorpusWriter *self,
                                           PyObject *args) {
  char *name;
  char *data;
...
static PyMethodDef IndexedCorpusWriter_methods[] = {
    {"write", (PyCFunction)IndexedCorpusWriter_write, METH_VARARGS,
     "Write corpus data"},
    {NULL} /* Sentinel */
};
...
typedef struct {
  PyObject_HEAD indexedcorpus::ActCorpusReader *corpusReader;
} CorpusReader;
...
static struct PyModuleDef cModPyDem = {
    PyModuleDef_HEAD_INIT, "indexcorpus", /* name of module */
    "", /* module documentation, may be NULL */
    -1, /* size of per-interpreter state of the module, or -1 if the module
           keeps state in global variables. */
    IndexedCorpusMethods};

PyMODINIT_FUNC PyInit_indexedcorpus(void) {
  PyObject *m = NULL;
  ...
  PyModule_AddObject(m, "IndexedCorpusReader",
                     (PyObject *)&IndexedCorpusReaderType);
  Py_INCREF(&IndexedCorpusWriterType);
  PyModule_AddObject(m, "IndexedCorpusWriter",
                     (PyObject *)&IndexedCorpusWriterType);
  return m;
}
