...
declare function local:head-root-string2($constituent as element(node)) as xs:string 
  {     
        if ($constituent[@cat="pp" and node[@rel="obj1"]]) then
            concat(local:head-root-string($constituent),
                   "+",
                   local:head-root-string($constituent/node[@rel="obj1"])) 
         else 
            local:head-root-string($constituent)
  };
...
let $triples := for $node in /alpino_ds//node[not(@cat="mwu") and 
    ...
    for $nd in ($node/node[not(node) or node[@rel="hd" or @rel="mwp"]],  $node/node[@cat="conj"]/node[@rel="cnj" and ( not(node) or node[@rel="hd" or @rel="mwp"])])
        let $droot := local:head-root-string2($nd)
