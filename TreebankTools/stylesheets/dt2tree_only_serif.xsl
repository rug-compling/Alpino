...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="/">
    <xsl:variable name="layoutTree">
      <xsl:apply-templates select="/alpino_ds/node/node[@cat]" mode="xml2layout"/>
    </xsl:variable>
  ...
  <xsl:template name="layout2svg">
    ...
    <!-- Create SVG wrapper -->
    <svg:svg viewBox = "{-1 * $xunit} {-1 * $yunit} {sum($layout/node/@width) * 2 * $xunit + $xunit} {$maxDepth * 2 * $yunit + 4 * $yunit}" style="text-anchor:middle;font-family:serif">
