  ...
  <xsl:template match="node" mode="xml2layout">
    ...
    <xsl:variable name="label">
      <xsl:choose>
        ...
        <xsl:when test = "string-length(@postag) &gt; string-length(@root) and string-length(@postag) &gt; string-length(@pos) and string-length(@postag) &gt; string-length(@rel)">
