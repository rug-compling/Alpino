...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="node" mode="xml2layout">
    ...
    <xsl:variable name="label">
      <xsl:choose>
        <xsl:when test = "@pt and string-length(@pt) &gt; string-length(@word) and string-length(@pt) &gt; string-length(@rel) and string-length(@pt) &gt; string-length(@lemma)">
          <xsl:value-of select="@pt"/>
        </xsl:when>
        <xsl:when test = "not(@pt) and string-length(@pos) &gt; string-length(@word) and string-length(@pos) &gt; string-length(@rel) and string-length(@pos) &gt; string-length(@root)">
          <xsl:value-of select="@pos"/>
        </xsl:when>
        <xsl:when test = "@pt and string-length(@word) &gt; string-length(@rel) and string-length(@word) &gt; string-length(@lemma)">
          <xsl:value-of select="@word"/>
        </xsl:when> 
        <xsl:when test = "not(@pt) and string-length(@word) &gt; string-length(@rel) and string-length(@word) &gt; string-length(@root)">
          <xsl:value-of select="@word"/>
        </xsl:when> 
       <xsl:when test = "@pt and string-length(@lemma) &gt; string-length(@rel)">
          <xsl:value-of select="@lemma"/>
        </xsl:when>
        <xsl:when test = "not(@pt) and string-length(@root) &gt; string-length(@rel)">
  ...
  <xsl:template match = "node" mode = "layout2svg">
    ...
    <!-- Draw label of node -->
    <svg:text x = "{$x}" y = "{$y - 30}" font-style="italic" text-anchor="middle">
      <xsl:if test ="@rel != 'top'">
      <xsl:value-of select="@rel"/>
      </xsl:if>
    </svg:text>
    ...
    <xsl:if test = "@root and not(@lemma) and not(@root=@word)">
      <svg:text x = "{$x}" y = "{$y + 25}" text-anchor="middle">
        <svg:tspan>
        <xsl:value-of select="@root"/>
        </svg:tspan>
