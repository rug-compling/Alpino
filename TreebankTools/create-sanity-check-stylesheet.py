...
def sanity_checks():
   ...
   check('//node[@cat="cp" and not(@rel="obcomp") and node[@rel="cmp" and @pos="comparative"]]','comparative but not an obcomp?')
   check('//node[count(node) = 1 and not(@cat="top")]', 'Een knoop  met maar 1 dochter')
   check('//node[@pos and not(@end = @begin + 1)]','leaf-node which is not a single word')
   ...
   check('//node[@pos="_" or @pos="UNKNOWN"]','wrong value for pos')
   ...
   check('//node[@root="inclusief" and @pos="prep"]','inclusief is not a prep but an adjective')
   check('//node[@root="incluis" and @pos="prep"]','incluis is not a prep but an adjective')
   check('//node[@root="uitgezonderd" and @pos="prep"]','uitgezonderd is not a prep but an adjective')
   check('//node[@root="voorbehouden" and @pos="prep"]','voorbehouden is not a prep but an adjective')
   check('//node[@root="bijgenaamd" and @pos="prep"]','bijgenaamd is not a prep but an adjective')
   check('//node[@root="daargelaten" and @pos="prep"]','daargelaten is not a prep but an adjective')
   check('//node[@root="gedoopt" and @pos="prep"]','gedoopt is not a prep but an adjective')
   check('//node[@root="geheten" and @pos="prep"]','geheten is not a prep but an adjective')
   check('//node[@root="genaamd" and @pos="prep"]','genaamd is not a prep but an adjective')
   check('//node[@root="getiteld" and @pos="prep"]','getiteld is not a prep but an adjective')
   check('//node[@root="genoemd" and @pos="prep"]','genoemd is not a prep but an adjective')
   check('//node[@root="inbegrepen" and @pos="prep"]','inbegrepen is not a prep but an adjective')
   check('//node[@root="zoals" and (@pos="prep" or @rel="hd" or ../@cat="pp")]','zoals is not a prep but a complementizer')
   check('//node[@root="als" and @pos="prep" and not(@rel="mwp")]','als is not a prep but a complementizer')
   check('//node[@cat="pp" and node[@rel="hd" and @pos="prep"] and not(node[@rel="obj1" or @rel="vc" or @rel="se"])]','preposition without object?')
   check('//node[not(@cat="pp") and node[@rel="hd" and @pos="prep"]]','preposition head, but not PP?')
   check('//node[@cat="np" and ./node[@cat="whrel"]]','whrel modifies an NP?')
   check('//node[node[@root="waard"] and node[@rel="me"]]','adjective waard takes obj1, not me')
   ...
   check('//node[node[@root="geen"] and node[@root="meer"]]','geen...meer no longer treated as constituent')
   ...
   check('//node[ ../@cat="np" and not(@rel="hd") and not(@rel="mod") and not(@rel="det") and not(@rel="app") and not(@rel="vc") and not(@rel="pc") and not(@rel="obcomp") and not(@rel="obj1") and not(@rel="su") and not(@rel="predm") and not(@rel="predc") and not(@rel="me") and not((@rel="obj2" or @rel="ld") and ../node[@rel="hd" and (@pos="verb" or @pos="adj")]) and not(@rel="svp" and ../node[@rel="hd" and (@pos="adj" or @pos="verb")]) and not(@rel="se" and ../node[@rel="hd" and (@pos="adj" or @pos="verb")])]','unexpected np-daughter.   Expected rels: hd mod det app vc pc obcomp obj1 su predm predc me')
   check('//node[node[@root="wil" or (@root="heb" and node[@rel="obj1" and not(@index)]) or @root="mag" or @root="zal" or @root="moet" or @root="hoef" or @root="kan" or @root="placht" or @root="begin" or @root="behoef" or @root="behoor" or @root="blijf" or @root="blijk" or @root="ben" or @root="word" or @root="dien" or @root="dreig" or @root="ga" or @root="heet" or @root="kom" or @root="lijk" or @root="mag" or @root="pleeg" or @root="schijn" ] and node[@rel="su" and not(@index)] and node[@rel="vc" and not(@cat="cp") and not(@cat="svan")]]','perhaps missing co-indexing??')
   check('//node[node[@rel="mod"] and node[(@root="heb" and node[@rel="obj1" and not(@index)]) or @root="zal" or @root="blijf" or @root="blijk" or @root="ben" or @root="word" or @root="ga" or @root="lijk" or @root="schijn" ] and node[@rel="su" and @index] and not(node[@rel="svp" or @rel="predc"]) and node[@rel="vc" and not(@cat="cp") and not(@cat="svan") and not(@cat="ahi")]]','modifier attached to auxiliary??')
   ...
   check('//node[node[@root="word" and @rel="hd"] and node[@rel="su" ] and node[@rel="vc" and @cat="ppart" and not(node[(@rel="obj1" or @rel="pobj1") and @index])]]','passive with subject but without main verb object?')
   check('//node[@pos="vg" and @rel="cnj"]','vg should be crd?')
   ...
   check('//node[not(@root="er") and @rel="sup" and ../node[@rel="su" and not(@cat="cp" or @cat="oti" or @cat="ti" or @cat="whsub" or @cat="conj")] and not(../node[@rel="hd" and (@root="ben" or @root="val")] and ../node[@rel="su" and @index])]','unexpected sup/su combination')
   check('//node[node[@rel="hd" and (@root="word" or @root="ben" or @root="zal" or @root="lijk" or @root="blijk" or @root="schijn" or @root="blijf" or @root="dunk")] and node[@rel="obj1"]]','unexpected obj1 with copula')
   ...
   check('//node[@cat="cp" and node[@rel="body" and @cat="inf"] and not(node[@rel="cmp" and @pos="comparative"])]','inf as body of cp?')
   ...
   check('//node[@pos="comp" and @root="om" and not(../@cat="oti")]','om comp without oti')
   check('//node[@pos="comp" and @root="te" and not(../@cat="ti")]','te comp without ti')
   check('//node[@pos="comp" and @root="dat" and not(../@cat="cp")]','dat comp without cp')
   check('//node[@pos="comp" and @root="of" and @rel="cmp" and not(../@cat="cp")]','of comp without cp')
   ...
   check('//node[@rel="whd" and @pos and not(@vwtype="vb" or @pt="bw" or @root="hoeveel")]','whd without wh postag?')
   ...
   check('//node[@rel="det" and @pos and not(@pt="lid" or @pt="vnw" or @pt="tw" or (@pt="n" and @naamval="gen") or @pt="spec" or @pt="adj" or @root="genoeg" or (@pt="n" and @ntype="eigen"))]','unexpected postag for determiner')
   check('//node[node[@getal="mv" and @rel="hd"] and node[@rel="det" and (@root="het" or @root="een" or @root="dit" or @root="dat")]]','plural head noun with singular determiner?')
   ...
   check('//node[@rel="hd" and ../@cat="advp" and @pt and not(@pt="bw" or @pt="n" or @pt="adj" or @pt="vnw" or @pt="tw")]','unexpected postag for head of advp')
   check('//node[@rel="hd" and ../@cat="pp" and @pt and not(@pt="vz" or @pt="spec" or @pt="bw" or @pt="adj")]','unexpected postag for head of pp')
