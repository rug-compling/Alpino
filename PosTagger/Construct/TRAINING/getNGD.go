...
func main() {
	...
	var baseTag, bigram, contextTrigram, firstTag, fourgram string
	var multiTagFirstWord, multiTagRestWords, newContext, oldContext string
	var prefixBigram, prefixTrigram, prefixUnigram string
...
MainLoop:
	for {
		...
		case tokenSentenceEnd:
			...
			// diversity
			if _, exists := tagFourgramFreq[fourgram]; !exists {
				prefixTrigramDiv[prefixTrigram]++
			}
			if _, exists := tagTrigramFreq[trigram]; !exists {
