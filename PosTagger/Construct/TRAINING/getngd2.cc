...
long int opt_c = 0, opt_f = 0, opt_g = 0;
...
//#$UNKNOWN_WORD_TAGPERCENTAGE = 0;  # percentage of most frequent tags to be
...
unordered_map<string, long int> wordFreq, wordTagFreq, contextTrigramFreq,
    tagFourgramFreq, tagTrigramFreq, tagBigramFreq, tagUnigramFreq,
    prefixTrigramFreq, prefixBigramFreq, prefixUnigramFreq, prefixTrigramDiv,
    prefixBigramDiv, lexicon;

unordered_set<string> usedContext;
...
void get_programname(char const *argv0), process_args(),
    errit(char const *format, ...), syntax();
char *get_arg();
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(buffer, BUFSIZE, fp) != NULL) {
        ...
        i = strlen(buffer);
        if (i >= BUFSIZE - 1)
            errit("Buffer full: increase BUFSIZE and recompile");
        ...
        if (n < 7) {
            for (i--; i >= 0; i--)
                if (buffer[i] == '\0')
                    buffer[i] = '|';
            cerr << "Parse failed for line " << lineno << ": " << buffer
                 << endl;
            continue;
        }
        word = w0;
        ...
        if (thirdTag[i] && i > 0 &&
            ...
            // if tag was converted from WOTAN format, multi-word tag will have
            // an extra number
            i = 0;
            while (i < (long int)baseTag.size() &&
                   isdigit((unsigned char)baseTag[i]))
                i++;
            if (i > 0 && baseTag[i] == '-')
            ...
            if (number == 1) {
                if (previousMultiTag != "<NONE>") {
                    addToLexicon(multiTagFirstWord, previousMultiTag,
                                 multiTagRestWords);
                }
                ...
                multiTagRestWords = "";
            } else if ((number == previousMultiNumber + 1) &&
                       (baseTag == previousMultiTag)) {
                previousMultiNumber = number;
                ...
                std::cerr << "Multiword tag error: " << word << " : "
                          << thirdTag << std::endl;
            }
        } else {
            if (previousMultiTag != "<NONE>") {
        ...
        if (sentenceKey != previousKey) {
            if (previousKey != "<NONE>") {
                // take care of ending of previous sentence (with sentenceEnd
                // for third tag)
                string fourgram = firstTag + "|" + secondTag + "|" +
                                  oldContext + "|" + SENTENCE_END;

                // store tag n-grams
                tagFourgramFreq[fourgram]++;

                // store context n-gram
                string contextTrigram =
                    oldContext + "|" + SENTENCE_END + "|" + DUMMY_CONTEXT;
        ...
        string fourgram =
            firstTag + "|" + secondTag + "|" + oldContext + "|" + thirdTag;
    ...
    string fourgram =
        firstTag + "|" + secondTag + "|" + oldContext + "|" + SENTENCE_END;
    ...
    for (unordered_map<string, long int>::iterator it = tagFourgramFreq.begin();
         it != tagFourgramFreq.end(); it++) {
    ...
    for (unordered_map<string, long int>::iterator it = tagTrigramFreq.begin();
         it != tagTrigramFreq.end(); it++) {
    ...
    for (unordered_map<string, long int>::iterator it = tagBigramFreq.begin();
         it != tagBigramFreq.end(); it++) {
    ...
    for (unordered_map<string, long int>::iterator it =
             prefixTrigramFreq.begin();
         it != prefixTrigramFreq.end(); it++) {
    ...
    for (unordered_map<string, long int>::iterator it =
             prefixBigramFreq.begin();
         it != prefixBigramFreq.end(); it++) {
    ...
    for (unordered_map<string, long int>::iterator it = tagFourgramFreq.begin();
        ...
        if ((*it).second > THRESHOLD) {
            double probability =
                ((double)(*it).second) / (double)prefixTrigramFreq[trigram];
    ...
    for (unordered_map<string, long int>::iterator it = tagTrigramFreq.begin();
        ...
        if ((*it).second > THRESHOLD) {
            double probability =
                ((double)(*it).second) / (double)prefixBigramFreq[bigram];
            long int probabilityInt = (long int)(-100.0 * log(probability));
            if (opt_g != 0 && probabilityInt < opt_g) {
    ...
    for (unordered_map<string, long int>::iterator it = tagBigramFreq.begin();
        ...
        if ((*it).second > THRESHOLD) {
            double probability =
                ((double)(*it).second) / (double)prefixUnigramFreq[unigram];
            long int probabilityInt = (long int)(-100.0 * log(probability));
            if (opt_g != 0 && probabilityInt < opt_g) {
    ...
    for (unordered_map<string, long int>::iterator it =
             contextTrigramFreq.begin();
         it != contextTrigramFreq.end(); it++) {
        if ((*it).second > THRESHOLD) {
            string trigram = (*it).first;
            size_t p = trigram.rfind('|');
            string bigram = trigram.substr(0, p);
            double probability =
                ((double)((*it).second)) / (double)tagBigramFreq[bigram];
            long int probabilityInt = (long int)(-100.0 * log(probability));
            if (opt_g != 0 && probabilityInt < opt_g) {
    ...
    // word-tags lexicon
    for (unordered_map<string, long int>::iterator it = lexicon.begin();
         it != lexicon.end(); it++) {
        // foreach $pair (keys %lexicon){
        // lexicon (word is printed twice as this format is required by fadd
        // morphology handling)
        if ((*it).second > THRESHOLD) {
    ...
    for (unordered_map<string, long int>::iterator it = wordTagFreq.begin();
         it != wordTagFreq.end(); it++) {
        string pair = (*it).first;
        size_t p = pair.find('|');
        string word = pair.substr(0, p);
        string tag = pair.substr(p + 1);
        ...
        if (wordFreq[word] > THRESHOLD) {
            long int probabilityInt =
                (long int)(-100.0 * log(((double)(*it).second) /
                                        (double)tagUnigramFreq[tag]));
            if (opt_g != 0 && probabilityInt < opt_g) {
                fprintf(WORD_TAG_FREQ, "%s|%li\n", pair.c_str(),
                        probabilityInt);
            }
        }
        // tag-word data (probability of seeing tag given word; used for
        // baseline)
        if ((*it).second > THRESHOLD) {
            long int probabilityInt =
                (long int)(-100.0 * log(((double)(*it).second) /
                                        (double)wordFreq[word]));
            if (opt_g != 0 && probabilityInt < opt_g) {
                fprintf(TAG_WORD_FREQ, "%s|%s|%lu\n", tag.c_str(), word.c_str(),
                        probabilityInt);
            }
        }
        // collect data for unknown word; collect tags for all words that occur
        // only once
    ...
    long int tagvalue = 0;
    string tagkey = "";
    for (unordered_map<string, long int>::iterator it = unknown.begin();
         it != unknown.end(); it++) {
    ...
    if (tagvalue > 0) {
        fprintf(WORD_TAG_LEX, "%s\t%s\t%s\n", UNKNOWN_WORD.c_str(),
                UNKNOWN_WORD.c_str(), tagkey.c_str());
    }
    fprintf(WORD_TAG_FREQ, "%s|%s|0\n", DUMMY_WORD.c_str(),
            SENTENCE_END.c_str());
    fprintf(WORD_TAG_FREQ, "%s|%s|0\n", DUMMY_WORD.c_str(),
            SENTENCE_START.c_str());

    // used context labels
    for (unordered_set<string>::iterator it = usedContext.begin();
         it != usedContext.end(); it++) {
